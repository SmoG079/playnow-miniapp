...
const slot = (index: number, court = 0, available = true): Slot => ({
  id: `${court}-${index}`,
  court,
  index,
  price: index > 3 ? 50 : 40,
...
describe("prototype booking rules", () => {
  ...
  it("truncates later slots when deselecting to preserve continuity", () => {
    expect(selectSlot([slot(0), slot(1), slot(2)], slot(1)).slots).toEqual([
      slot(0),
    ]);
  });
  it("combines date, level and feed filters", () => {
    const items = [
      { day: 0, level: 3, kind: "match" },
      { day: 1, level: 3, kind: "tournament" },
    ];
