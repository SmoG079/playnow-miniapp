...
const routes = { home: 'home/index', clubs: 'booking/club-list', venue: 'booking/venue-detail?id=1', activity: 'common/post-detail?id=1', publish: 'publish/post-create', profile: 'profile/index', chat: 'chat/index' }
;(async () => {
  const browser = await chromium.launch({ headless: true, channel: 'msedge' })
  const page = await browser.newPage({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 1 })
  const errors = [], externalRequests = [], results = []
  page.on('pageerror', e => errors.push(e.message))
  page.on('request', r => { if (/^https?:/.test(r.url()) && !r.url().startsWith(base)) externalRequests.push(r.url()) })
  let visitNumber = 0
  async function visit(name) { await page.goto(`${base}/?qa=${++visitNumber}#/pages/${routes[name]}`, { waitUntil: 'networkidle' }); await page.waitForTimeout(200) }
  async function snap(name, full = false) {
    // Anchor persistent controls to the end of long-page review exports only.
    await page.screenshot({ path: path.join(out, `${name}.png`), fullPage: full, animations: 'disabled', style: full ? '.app-shell{position:relative!important}.bottom-nav,.fixed-action{position:absolute!important;bottom:0!important}' : '' })
  }
  try {
    for (const width of [375,390,430]) {
      ...
      for (const name of Object.keys(routes)) {
        ...
        const dimensions = await page.evaluate(() => ({ w: document.documentElement.clientWidth, scroll: document.documentElement.scrollWidth }))
        assert.ok(dimensions.scroll <= dimensions.w + 1, `${name} overflows at ${width}`)
        const broken = await page.locator('uni-image img').evaluateAll(imgs => imgs.filter(i => !i.complete || i.naturalWidth === 0).length)
    ...
    await visit('home')
    await page.getByText('全部时间', { exact:false }).click()
    await page.locator('.prototype-sheet').getByText('今天', { exact:true }).click()
    await page.locator('.prototype-sheet').getByText('4.0', { exact:true }).click()
    ...
    results.push('slot selection, booking confirmation and profile record pass')
    await visit('activity')
    await page.getByText('加入这场球', { exact:true }).click()
    await page.locator('[role="status"]').getByText('报名成功，球场见！', { exact:false }).waitFor()
    await snap('joined')
    ...
    results.push('desktop phone framing, no runtime errors, no external API requests pass')
    fs.writeFileSync(path.join(out, 'verification.json'), JSON.stringify({ results, errors, externalRequests }, null, 2))
    console.log(JSON.stringify({ passed: results.length, errors, externalRequests }))
