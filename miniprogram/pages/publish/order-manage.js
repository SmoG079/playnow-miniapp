...
Page({
  ...
  onOrderTap(e) {
    ...
    this.setData({ selectedOrder: order, showDetail: true });
  },
  ...
  formatStatus(status) {
    const map = { pending: '待支付', paid: '已支付', completed: '已完成', cancelled: '已取消', refunding: '退款中', refunded: '已退款' };
    return map[status] || status;
  },

  statusColor(status) {
    const map = { pending: 'orange', paid: 'green', completed: 'blue', cancelled: 'gray', refunding: 'red', refunded: 'gray' };
    return map[status] || 'gray';
  },

  stopPropagation() {
    // Prevent event bubbling to parent card tap handler
  },
});
